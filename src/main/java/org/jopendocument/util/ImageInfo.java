 ...
 * Pixmap (PBM, PGM, PPM) and Adobe Photoshop (PSD). Added new method getMimeType() to return the
 * MIME type associated with a particular file format.</li>
 ...
 * encountered in an input image file / stream. Added two methods to return the physical width and
 * height of the image in dpi: {@link #getPhysicalWidthDpi()} and {@link #getPhysicalHeightDpi()}.
 ...
 * consideration. Less bytes than necessary may have been skipped, leading to flaws in the retrieved
 * information in some cases. Thanks to Bernard Bernstein for pointing that out. Released as 1.4.</li>
 * <li><strong>2004-02-29</strong> Added support for recognizing progressive JPEG and interlaced
 * PNG and GIF. A new method {@link #isProgressive()} returns whether ImageInfo has found that the
 ...
 * terminology) did not work (thanks to Franz Jeitler for pointing this out). Now it should work,
 * but only if the number of images is determined. This is because information on interlacing is
 * stored in a local image header. In theory, different images could be stored interlaced and
 * non-interlaced in one file. However, I think that's unlikely. Right now, the last image in the
 ...
 * Boolean.toString because it's a Java 1.4+ feature (thanks to Gregor Dupont). Changed delimiter
 * character in compact output from semicolon to tabulator (for better integration with cut(1) and
 * other Unix tools). Added some points to the <a
 * href="http://schmidt.devlib.org/image-info/index.html#knownissues">'Known issues' section of the
 ...
 * people. I give up on SWF. Please do not contact me about it anymore. Set package of ImageInfo
 * class to org.devlib.schmidt.imageinfo (a package was repeatedly requested by some users).
 * Released as 1.7.</li>
 * <li><strong>2006-02-23</strong> Removed Flash helper methods which weren't used elsewhere.
 * Updated skip method which tries "read" whenever "skip(Bytes)" returns a result of 0. The old
 * method didn't work with certain input stream types on truncated data streams. Thanks to Martin
 * Leidig for reporting this and sending in code. Released as 1.8.</li>
 * </li>
 * <li><strong>2006-11-13</strong> Removed check that made ImageInfo report JPEG APPx markers
 * smaller than 14 bytes as files in unknown format. Such JPEGs seem to be generated by Google's
 * Picasa application. First reported with fix by Karl von Randow. Released as 1.9.</li>
 * </ul>
 * 
...
public class ImageInfo {
     ...
     * GIFs and count the number of images (GIFs with more than one image are animations). It is
     ...
     * {@link #setInput(InputStream)} or {@link #setInput(DataInput)}. If true is returned, the
     * file format was known and information on the file's content can be retrieved using the
    ...
    private boolean checkGif() throws IOException {
        ...
        do {
            ...
            case (0x2c): // image separator
            {
                 ...
                 * getShortLittleEndian(a, 6); System.out.println("LOCAL: " + locWidth + " x " +
                 * locHeight);
                 */
     ...
     * <code>true</code> as argument, returns the number of comments retrieved from the input
     * image stream / file. Any number &gt;= 0 and smaller than this number of comments is then a
     ...
     * information is not available. Assumes that {@link #check} has been called successfully.
     * 
    ...
    private static void printCompact(String sourceName, ImageInfo imageInfo) {
        final String SEP = "\t";
        System.out.println(sourceName + SEP + imageInfo.getFormatName() + SEP + imageInfo.getMimeType() + SEP + imageInfo.getWidth() + SEP + imageInfo.getHeight() + SEP + imageInfo.getBitsPerPixel()
                + SEP + imageInfo.getNumberOfImages() + SEP + imageInfo.getPhysicalWidthDpi() + SEP + imageInfo.getPhysicalHeightDpi() + SEP + imageInfo.getPhysicalWidthInch() + SEP
     ...
     * <code>false</code>. This is a special option because some file formats require running
     * over the entire file to find out the number of images, a rather time-consuming task. Not all
     * file formats support more than one image. If this method is called with <code>true</code>
     * as argument, the actual number of images can be queried via {@link #getNumberOfImages()}
     * after a successful call to {@link #check()}.
     * 
